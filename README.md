...
`CreateAuthOptions` is a closed type at the top level: a misspelled key (`magicLinks:` for `magicLink:`) is a type error rather than a silently ignored option. The four passthroughs to Better Auth — `betterAuth`, `session`, `rateLimit` and `advanced` — are intentionally loose records, so keys nested under them are not checked against Better Auth's types; consult Better Auth's own documentation for those. Anything Better Auth accepts that is not listed below goes through `betterAuth`.
...
| `secondaryStorage` | `SecondaryStorage`                                                   | built from `kv`          | Replaces the KV-backed secondary storage; satisfies the `kv` requirement, but session-cache invalidation still needs `kv`. |
| `session`          | `Partial<BetterAuthOptions['session']>`                              | cookie cache on          | Session settings. Merge rule below.                                                                                        |
| `rateLimit`        | `Partial<BetterAuthOptions['rateLimit']>`                            | KV storage               | Rate-limiter settings. Merge rule below.                                                                                   |
| `hooks`            | `{ before?, after? }`                                                | none                     | Your hooks, composed with the package's own (method restriction, cache invalidation). Merge rule below.                    |
| `advanced`         | `Partial<BetterAuthOptions['advanced']>`                             | schema validation off    | Better Auth's `advanced` settings. Merge rule below.                                                                       |

**Merge rule.** `session`, `rateLimit`, `hooks` and `advanced` each also exist under `betterAuth`, and all four follow the same rule: the package's default, then the top-level option, then `betterAuth.<name>`, shallow-merged field by field with the later layer winning. So `betterAuth` is always the last word (its "merged last, can override anything" rule), without having to restate the fields you do not change; e.g. `rateLimit: { max: 5 }` plus `betterAuth: { rateLimit: { window: 30 } }` yields `{ storage: 'secondary-storage', max: 5, window: 30 }`. The package's own hooks are then composed around the merged `hooks`, and `advanced.database.validateSchema` stays off unless you set it.
