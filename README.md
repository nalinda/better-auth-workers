...
`banUser` does what the admin plugin's ban does: it sets `banned`, `banReason` (default `'No reason'`) and `banExpires` (permanent without `expiresIn`, in seconds), and revokes every session the user has. It also deletes each of those sessions from the session client's KV cache, so `requireSession` in any Worker sharing the namespace refuses the user on their next request. Other Cloudflare locations can take up to about 60 seconds to see a KV delete; if you re-check `banned` yourself in a `requireSession` predicate, that closes the gap. A banned user who tries to sign in gets `BANNED_USER`. `unbanUser` clears all three fields. Both return `found: false` and change nothing for an unknown id.
