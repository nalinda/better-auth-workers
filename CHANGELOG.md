...
- `phone.awaitDelivery`: `send-otp` waits for `sendOTP`. A failure answers `502 OTP_DELIVERY_FAILED` and deletes the undelivered code (unless a resend has already replaced it); an `OTPDeliveryError` thrown by `sendOTP` answers with its own code and status, plus `retryAfter` and a `Retry-After` header.
...
- Better Auth's rate-limit `429` now carries `code: 'RATE_LIMITED'`, and an unexpected failure (an empty `500`, or an error thrown out of the handler) answers a JSON `500` with `code: 'INTERNAL_ERROR'`, like every other error.
