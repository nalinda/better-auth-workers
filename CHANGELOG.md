...
- `phone.awaitDelivery`: `send-otp` waits for `sendOTP`. A failure answers `502 OTP_DELIVERY_FAILED` and deletes the undelivered code (best-effort); an `OTPDeliveryError` thrown by `sendOTP` answers with its own code and status, plus `retryAfter` and a `Retry-After` header.
...
- Better Auth's rate-limit `429` now carries `code: 'RATE_LIMITED'`, and an unexpected failure's empty `500` carries `code: 'INTERNAL_ERROR'`, like every other error.
