...
- `phone.awaitDelivery`: `send-otp` waits for `sendOTP`. A failure answers `502 OTP_DELIVERY_FAILED` and deletes the undelivered code only, so a newer code from a resend is untouched and the user's previous code works again; an `OTPDeliveryError` thrown by `sendOTP` answers with its own code and status, plus `retryAfter` and a `Retry-After` header.
